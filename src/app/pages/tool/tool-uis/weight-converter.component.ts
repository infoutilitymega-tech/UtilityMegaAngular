      ...
      <div class="results-section" *ngIf="value">
        ...
        <div class="results-grid">
          <div class="group-section">
            <div class="group-title">⚖️ Metric</div>
      ...
      <div class="ref-table">
        ...
        <div class="table-scroll">
          <table class="conv-table">
            <thead><tr><th>Unit</th><th>= 1 Kilogram</th><th>= 1 Pound</th><th>= 1 Ounce</th></tr></thead>
            <tbody>
              <tr *ngFor="let u of tableUnits()">
                <td><strong>{{ u.label }}</strong></td>
                <td>{{ fmtRef(1 / u.toGrams * 1000) }}</td>
                <td>{{ fmtRef(453.592 / u.toGrams) }}</td>
                <td>{{ fmtRef(28.3495 / u.toGrams) }}</td>
              </tr>
            </tbody>
          </table>
        </div>
...
export class WeightConverterComponent {
  ...
  metricUnits = [
    { key: 't', label: 'Metric Ton (t)', toGrams: 1_000_000 },
    { key: 'kg', label: 'Kilogram (kg)', toGrams: 1000 },
    { key: 'g', label: 'Gram (g)', toGrams: 1 },
    { key: 'mg', label: 'Milligram (mg)', toGrams: 0.001 },
    { key: 'ug', label: 'Microgram (μg)', toGrams: 0.000001 },
  ];

  imperialUnits = [
    { key: 'ton', label: 'Short Ton (US)', toGrams: 907184.74 },
    { key: 'lt', label: 'Long Ton (UK)', toGrams: 1016046.91 },
    { key: 'lb', label: 'Pound (lb)', toGrams: 453.592 },
    { key: 'oz', label: 'Ounce (oz)', toGrams: 28.3495 },
  ];

  otherUnits = [
    { key: 'st', label: 'Stone (st)', toGrams: 6350.29 },
    { key: 'ct', label: 'Carat (ct)', toGrams: 0.2 },
    { key: 'gr', label: 'Grain (gr)', toGrams: 0.0648 },
  ];
  ...
  popularPairs = [
    ...
    { from: 't', to: 'ton', label: 'Metric T → Short T' },
  ];
  ...
  convert() {
    ...
    const inGrams = this.value * base.toGrams;
    const out: Record<string, number> = {};
    this.allUnits().forEach(u => { out[u.key] = inGrams / u.toGrams; });
