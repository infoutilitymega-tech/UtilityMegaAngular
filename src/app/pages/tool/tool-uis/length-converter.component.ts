...
@Component({
  ...
  template: `
    <div class="lc-wrap">
      ...
      <div class="popular-row">
        <button *ngFor="let p of popularPairs" class="pop-btn" (click)="setConversion(p.from, p.to)">
          {{ p.label }}
        </button>
      </div>
      ...
      <div class="ref-table">
        ...
        <div class="table-scroll">
          <table class="conv-table">
            <thead><tr><th>Unit</th><th>= 1 Meter</th><th>= 1 Foot</th><th>= 1 Inch</th></tr></thead>
            <tbody>
              <tr *ngFor="let u of tableUnits()">
                <td><strong>{{ u.label }}</strong></td>
                <td>{{ fmtRef(1 / u.toMeters) }}</td>
                <td>{{ fmtRef(0.3048 / u.toMeters) }}</td>
                <td>{{ fmtRef(0.0254 / u.toMeters) }}</td>
              </tr>
            </tbody>
          </table>
        </div>
  ...
  styles: [`
    ...
    .sb-fill.from-bar{opacity:1;background:var(--primary)}
...
export class LengthConverterComponent {
  ...
  metricUnits = [
    { key: 'km', label: 'Kilometer (km)', toMeters: 1000 },
    { key: 'm', label: 'Meter (m)', toMeters: 1 },
    { key: 'cm', label: 'Centimeter (cm)', toMeters: 0.01 },
    { key: 'mm', label: 'Millimeter (mm)', toMeters: 0.001 },
    { key: 'um', label: 'Micrometer (μm)', toMeters: 0.000001 },
    { key: 'nm', label: 'Nanometer (nm)', toMeters: 1e-9 },
  ];

  imperialUnits = [
    { key: 'mi', label: 'Mile (mi)', toMeters: 1609.344 },
    { key: 'yd', label: 'Yard (yd)', toMeters: 0.9144 },
    { key: 'ft', label: 'Foot (ft)', toMeters: 0.3048 },
    { key: 'in', label: 'Inch (in)', toMeters: 0.0254 },
  ];

  otherUnits = [
    { key: 'nmi', label: 'Nautical Mile', toMeters: 1852 },
    { key: 'ly', label: 'Light Year', toMeters: 9.461e15 },
    { key: 'au', label: 'Astro. Unit', toMeters: 1.496e11 },
  ];

  popularPairs = [
    { from: 'm', to: 'ft', label: 'm → ft' },
    { from: 'km', to: 'mi', label: 'km → mi' },
    { from: 'in', to: 'cm', label: 'in → cm' },
    { from: 'ft', to: 'cm', label: 'ft → cm' },
    { from: 'mi', to: 'km', label: 'mi → km' },
  ];

  tableUnits() {
  return [...this.metricUnits, ...this.imperialUnits.slice(0, 3)];
}
  ...
  convert() {
    ...
    const inMeters = this.value * base.toMeters;
    const out: Record<string, number> = {};
    this.allUnits().forEach(u => { out[u.key] = inMeters / u.toMeters; });
    this.results.set(out);
  }

  setConversion(from: string, _to: string) { this.fromUnit = from; this.convert(); }
  ...
  scaleUnits() {
    ...
    const units = [...this.metricUnits.slice(0, 4), ...this.imperialUnits.slice(0, 3)];
