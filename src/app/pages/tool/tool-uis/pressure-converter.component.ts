        ...
        <div class="mi-unit">
          <select [(ngModel)]="fromUnit" (change)="convert()" class="unit-sel">
            ...
            <optgroup label="Atmosphere">
              <option *ngFor="let u of atmosUnits" [value]="u.key">{{ u.label }}</option>
            </optgroup>
          </select>
        </div>
      </div>
      ...
      <div class="results-section" *ngIf="value">
        ...
        <div class="results-grid">
          ...
          <div class="group-section">
            ...
            <div class="result-item" *ngFor="let u of atmosUnits" [class.from-unit]="u.key===fromUnit">
              <span class="ri-unit">{{ u.label }}</span>
              <span class="ri-val">{{ fmt(results()[u.key]) }}</span>
              <button class="ri-copy" (click)="copyVal(u.key)" [class.copied]="copiedKey()===u.key">{{ copiedKey()===u.key?'✓':'📋' }}</button>
            </div>
      ...
      <div class="ref-table">
        ...
        <div class="table-scroll">
          <table class="conv-table">
            <thead><tr><th>Unit</th><th>= 1 Pascal</th><th>= 1 Bar</th><th>= 1 atm</th></tr></thead>
            <tbody>
              <tr *ngFor="let u of tableUnits()">
                <td><strong>{{ u.label }}</strong></td>
                <td>{{ fmtRef(1 / u.toPascals) }}</td>
                <td>{{ fmtRef(100000 / u.toPascals) }}</td>
                <td>{{ fmtRef(101325 / u.toPascals) }}</td>
              </tr>
            </tbody>
          </table>
        </div>
...
export class PressureConverterComponent {
  ...
  metricUnits = [
    { key: 'pa', label: 'Pascal (Pa)', toPascals: 1 },
    { key: 'hpa', label: 'Hectopascal (hPa)', toPascals: 100 },
    { key: 'kpa', label: 'Kilopascal (kPa)', toPascals: 1000 },
    { key: 'mpa', label: 'Megapascal (MPa)', toPascals: 1_000_000 },
    { key: 'bar', label: 'Bar', toPascals: 100_000 },
    { key: 'mbar', label: 'Millibar (mbar)', toPascals: 100 },
  ];

  imperialUnits = [
    { key: 'psi', label: 'PSI (lb/in²)', toPascals: 6894.76 },
    { key: 'psf', label: 'PSF (lb/ft²)', toPascals: 47.8803 },
    { key: 'inhg', label: 'inHg (inch Hg)', toPascals: 3386.39 },
  ];

  atmosUnits = [
    { key: 'atm', label: 'Atmosphere (atm)', toPascals: 101325 },
    { key: 'torr', label: 'Torr', toPascals: 133.322 },
    { key: 'mmhg', label: 'mmHg (mm of Hg)', toPascals: 133.322 },
    { key: 'at', label: 'Technical Atm (at)', toPascals: 98066.5 },
  ];

  popularPairs = [
    { from: 'bar', label: 'bar → PSI' },
    { from: 'psi', label: 'PSI → bar' },
    { from: 'atm', label: 'atm → Pa' },
    { from: 'kpa', label: 'kPa → PSI' },
    { from: 'inhg', label: 'inHg → hPa' },
  ];

  tableUnits() { return [...this.metricUnits.slice(0, 4), ...this.imperialUnits, ...this.atmosUnits.slice(0, 2)]; }
  allUnits() { return [...this.metricUnits, ...this.imperialUnits, ...this.atmosUnits]; }
  ...
  convert() {
    ...
    const inPascals = this.value * base.toPascals;
    const out: Record<string, number> = {};
    this.allUnits().forEach(u => { out[u.key] = inPascals / u.toPascals; });
  ...
  scaleUnits() {
    ...
    const units = [...this.metricUnits.slice(0, 3), ...this.imperialUnits.slice(0, 2), ...this.atmosUnits.slice(0, 2)];
